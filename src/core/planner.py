...
class Planner:
    ...
    def _resolve_prompts_effective(
        ...
        """Read the run's own, already-frozen prompts.

        Run configuration is frozen at creation
        (docs/contracts/configuration-hierarchy.md: "Run configuration is
        frozen at creation; never changes") — `ConfigResolver.build_run_config_dict`
        already resolves CLI > experiment > None and bakes the result
        directly onto the run's own `config` at `--add-run` time (see
        `_resolve_cli_or_experiment`). There is no execution-time
        inheritance step left to perform: the run's own persisted value
        IS the final answer, whether it came from an explicit
        --system-prompt/--user-prompt, from inheriting the experiment's
        default at creation time, or from None (either never set
        anywhere, or explicitly cleared via --system-prompt system-default,
        which the run's own creation-time resolution already applied).

        Previously this method took a SECOND, execution-time fallback to
        `experiment_row` (`run_config.get(...) or exp_config.get(...)`)
        whenever the run's own value was falsy — indistinguishable from
        "never resolved" and "explicitly cleared via system-default", so
        it silently re-inherited the experiment's prompt even for a run
        created with `--system-prompt system-default`, defeating that
        flag's whole purpose. Fixed 2026-08-21 (system-default semantics
        audit) — see docs/status/known-issues.md.

        Args:
            experiment_row: Experiment database row — accepted for
                signature stability with `build_plan`'s call site; not
                read here (the run's own config is authoritative, per the
                docstring above).
        ...
        return Prompts(
            system=run_config.get("SYSTEM_PROMPT"),
            user=run_config.get("USER_PROMPT"),
        )
    ...
    def _build_model_config(self, variant_row: sqlite3.Row) -> ModelConfig:
        ...
        #
        # reasoning_effort passes through EXACTLY as persisted — 'none' is
        # a real, explicit MODEL_REASONING_EFFORT value (disables
        # reasoning on the API side by sending effort='none'), not a
        # sentinel to collapse to Python None here. Collapsing it used to
        # silently re-enable reasoning whenever a MODEL_MAX_TOKENS_REASONING
        # value was also present (inherited or explicit) — 'none' would
        # vanish, the conflict check downstream would no longer see a
        # concrete reasoning_effort, and reasoning.max_tokens would be
        # sent despite the user asking for 'none'. ConfigResolver's
        # mode-suppression (_resolve_reasoning_pair) now guarantees at
        # most one of MODEL_REASONING_EFFORT/MODEL_MAX_TOKENS_REASONING is
        # ever persisted non-null, so no collapsing is needed here at all
        # — see docs/status/known-issues.md, 2026-08-21.
