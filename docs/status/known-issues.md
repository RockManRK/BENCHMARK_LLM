...
### ✅ [Moved to Resolved Issues] `--reasoning-tokens 0` / `--reasoning-tokens system-default` silently falls through to `--max-reasoning`

Found 2026-08-20 by the Essence Guardian during the `bcllm_model.py` Typer conversion review, flagged as adjacent-but-out-of-scope for that slice. Fully resolved 2026-08-21 as part of the reasoning effort/tokens exclusivity checkpoint, which removed `--max-reasoning` entirely (it was a true, undocumented synonym of `--reasoning-tokens`) rather than teaching the `or`-fallback to respect `0`/`FORCE_SYSTEM_DEFAULT` — the fallback itself no longer exists. See the Resolved Issues entry below ("Reasoning effort/tokens exclusivity, `--max-reasoning` removal, and 3 related `system-default` inheritance bugs") for the complete fix and regression coverage.

### 🟡 Test-suite debt groups G1–G9 (external marco 4C read-only audit, 2026-08-21) — registered, not fixed

**Severity:** Low-to-Medium, ungraded per-group — this project only has the external audit's own group labels (G1–G12) and short descriptions relayed by the user; the full audit report itself was not made available to this session, so most groups are registered here from description only, not independently verified.
**Scope:** G1–G7 and G9 are registered as suite debt per explicit user instruction — **not investigated, not fixed** in this pass, deferred until either prioritized directly or found to block the Execute path in the main environment (per the same instruction, "exceto se algum bloquear diretamente o caminho de Execute no ambiente principal" — none of the pytest/cli_suite runs this session showed the Execute path itself blocked; the pre-existing baseline `48 failed/18 skipped/39 errors` is unchanged before and after this checkpoint).
**G8 — investigated, then the 6 `AsyncWriter` tests rewritten against a confirmed normative contract (user decision, 2026-08-21); no production code changed.** Two adjacent-but-distinct pre-existing failure clusters, both already part of the standing 48-failed baseline (not new) before this fix:
1. **`tests/unit/core/test_async_orchestrator_duration.py` (6 failures, all `AttributeError: 'AsyncOrchestrator' object has no attribute '_update_run_statuses'`).** Investigated: this method does not exist anywhere in the current codebase — `AsyncOrchestrator.execute()` (see its own docstring, step 9) instead calls `RunFinalizer.finalize_run()`, explicitly commented in `async_orchestrator.py` as "sole owner of runs.status/duration". `RunFinalizer` computes duration via a real SQL query summing latency from persisted `responses` rows (`src/core/run_finalizer.py`), a materially different (and more robust — reads what was actually persisted rather than accumulating from an in-memory results list) mechanism than what this orphaned test file expects. **`tests/unit/core/test_run_finalizer.py` (7 tests) independently and correctly covers the REAL, current mechanism — all 7 pass.** Conclusion: `test_async_orchestrator_duration.py` is a stale test file left over from a refactor that moved this responsibility out of `AsyncOrchestrator`, not a sign that progress-preservation is actually broken — the real mechanism is tested and working. Left as-is (not rewritten/deleted) — out of the approved scope for this pass, which covered `AsyncWriter` (G8's "resiliência de escrita") specifically.
2. **`tests/test_async_writer.py` (6 failures across `TestAsyncWriterFailureResilience`/`TestAsyncWriterStats`/`TestAsyncWriterSentinel`) — root causes confirmed on independent investigation:** `test_consume_survives_all_db_failures`/`test_consume_survives_db_write_failure`/`test_task_done_called_for_each_item` all constructed `AsyncWriter(queue, mock_db)` **outside** the `patch('src.core.async_writer.ResultWriter', ...)` block meant to substitute it, so the real (unpatched) `ResultWriter` ran against a bare `MagicMock()`, producing incidental `TypeError`s unrelated to the resilience logic under test. `test_stats_track_written_errors`/`test_stats_after_partial_consume` selected failures by a global call counter ("every 3rd write fails") that conflated a retried item's earlier failed attempts with entirely different items. `test_sentinel_at_start_exits_immediately` used brittle whole-dict equality against a `stats` shape that had grown two more keys (`aborted`/`abort_info`) since the test was written.

   **This same investigation also surfaced a real coverage gap** (not a bug in `AsyncWriter` itself, which reads as structurally sound — `_write_result_with_retry`'s retry/backoff/abort comparisons are all against real `int`/`float` constants, never anything DB-derived): the suite had **no test actually proving the fail-fast contract** — that a permanent write failure stops the run rather than silently skipping ahead. Given a **normative decision** (user, 2026-08-21): **AsyncWriter keeps its current fail-fast behavior** — after a permanent write failure (retries exhausted): run the configured retries; set `aborted=True`; fill `abort_info`; emit the CRITICAL `WRITE_ABORT` event; stop consuming; stop new items from being produced; finalize the Run via the existing `RunFinalizer` flow; do **not** silently continue processing later items. Production explicitly NOT changed to implement "per-item resilience" instead.

   **Fix:** all 6 tests rewritten to validate the confirmed real contract, using a new `_SelectiveFailureResultWriter` helper (wraps the REAL `ResultWriter`, real SQLite writes on success, injecting failures keyed by `item_id` + per-item attempt number, never a global call counter — real SQLite in-memory `db_connection` fixture used throughout instead of raw unconfigured `MagicMock`, per instruction, except the one case where nothing ever touches the DB at all): `test_transient_failure_then_retry_succeeds` (one failed attempt, retry succeeds, `written`+1/`errors`+0/`aborted=False`); `test_permanent_failure_on_second_item_fails_fast` (first item persisted, second permanently fails, `written=1`/`errors=1`/`aborted=True`/`abort_info` correctly identifies the failing item by its embedded message + run/variant/snapshot fields, third item and sentinel never even dequeued — `queue.qsize()` proves it); `test_stats_distinguish_retry_attempts_from_item_outcomes` (a transient retry doesn't inflate `errors`); `test_stats_select_failing_item_by_id_not_position` (failure targeting is by `item_id`, and fail-fast is reconfirmed with a different item position); `test_sentinel_at_start_exits_immediately` (rewritten to check individual fields, not whole-dict equality); `test_task_done_called_for_each_item_on_the_happy_path` (split off the happy-path claim, now proven via `queue.join()` completing promptly — the public `asyncio.Queue` API for this, not a private attribute) + new `test_task_done_not_called_for_the_item_that_triggers_abort` (documents, via a `queue.join()` timeout, the REAL and INTENTIONALLY-UNCHANGED task_done() gap on the aborting item — inert today since `AsyncOrchestrator` never calls `queue.join()` after an abort, but now explicit so a future `queue.join()` addition doesn't silently hang). `TestAsyncWriterFailureResilience` renamed to `TestAsyncWriterFailFastContract` to match the confirmed contract's own name. **16/16 tests pass** (was 9 passed/6 failed; net +1 test from splitting the task_done happy-path/abort-path claims apart).

   **`_write_error` duplicate-row theoretical risk — registered as debt, NOT fixed:** `ResultWriter._write_error` (`src/core/result_writer.py`) computes `next_attempt` via `SELECT MAX(attempt_number)+1 ... WHERE response_id = ?`, INSERTs, then calls `self.db_connection.commit()` (line 349), followed by one `self._logger.debug(f"WRITE_COMPLETE | ...")` call (lines 351-354) before the method returns — **corrected 2026-08-21 after an Essence Guardian verification pass found the first version of this entry wrong: `commit()` is NOT the method's last statement, the debug-log call is.** If an exception were raised anywhere between `commit()` succeeding and the method actually returning control to its caller (`AsyncWriter._write_result_with_retry`, which retries on any `Exception`) — including inside the trailing f-string formatting/logger call itself — the caller could treat an already-committed write as a failure and retry it, inserting a SECOND error row (`next_attempt` incremented again) for what was really one failed API call. The window is two statements wide (the debug-log call, then the implicit `return`), not one — still narrow and unlikely (a logging call raising is itself an edge case), but a real correction to the original claim's precision, not just a wording nitpick. **Per explicit instruction: this window is NOT to be closed without reproduction or an approved design** — registered here as a theoretical risk only, not a confirmed bug.
**Impact:** The fail-fast contract that was already true in `AsyncWriter`'s code is now actually locked in by tests, closing the real coverage gap this investigation found. No production behavior changed. `test_async_orchestrator_duration.py` remains untouched, stale debt (out of this pass's approved scope).
**Not fixed by this change:** G1–G7, G9, and `test_async_orchestrator_duration.py` remain untouched. The `_write_error` duplicate-row window remains unfixed by explicit instruction. `RetryHandler`/`Exception | callable`/`click`-import fixes (see the dedicated entry above) are unrelated code-quality bugs the same audit surfaced, already fixed.
...
### ✅ [Moved to Resolved Issues] `--remove-question` performed a real hard delete, contradicting its own "soft delete" docstring and the immutability contract

Resolved 2026-08-20, same day it was found by the Essence Guardian's marco 4A review — decision: remove the command entirely, not build soft-delete. See the Resolved Issues entry below ("`--remove-question` removed from the system — QuestionSnapshot stays fully immutable"). Left as a pointer here rather than deleted, matching the convention used for the entries above.
...
**Python version:** Running/tested this session: **3.14.2**. Declared minimum in `setup.py` at the time: `python_requires=">=3.10"`. No evidence in this project of the 3.10 floor itself having been exercised (only 3.14.2 has real test-run evidence) — the floor was a declared compatibility claim, not a verified one.

**Updated 2026-08-21 (user decision, definitive):** the unverified-floor gap above is now closed by narrowing the claim to match reality, not by testing the gap. `setup.py`'s `python_requires` raised to `>=3.14` — the project prioritizes its real, currently-running environment over untested backward compatibility; Python 3.10–3.13 are no longer a support target, and no version matrix will be run or maintained. `docs/architecture/overview.md` and `docs/guides/ai-development-workflow.md` updated to match (`Python >=3.14 (validated: 3.14.2)`). Explicitly not a compatibility-shim decision — the two unrelated, objectively-incorrect type annotations found in the same review pass (`AsyncOrchestrator`'s `RetryHandler` reference only available via a local import inside a different method than the one annotating a parameter with it, and a test's `Exception | callable` — `callable` is the builtin function, not a type) were fixed as ordinary code-quality bugs, not as part of any version-compatibility effort. See the CLI Typer migration entries for the `RetryHandler`/`Callable` fix details and `tests/unit/cli/test_remove_question_removed.py`'s direct `import click` replaced with `typer.testing.CliRunner` (no new dependency added).
...
### ✅ `--remove-question` removed from the system — QuestionSnapshot stays fully immutable

**Resolved:** 2026-08-20 (same day found, before marco 4B — user decision, definitive)
**Description:** `--remove-question` hard-deleted rows via `SnapshotRepository.delete()` despite its own docstring claiming "soft delete," contradicting `docs/contracts/immutability.md` §1. Rather than build real soft-delete (new column, migration), the decision was to remove the command entirely: QuestionSnapshot is immutable, an experiment can only grow by adding snapshots — a question already added can never be removed, disabled, or retroactively hidden.
...
**Impact:** Marco 4A complete. `bcllm_model.py`, `bcllm_run.py`, `bcllm_provider.py`, `bcllm_execute.py`, `bcllm_review.py`, `bcllm_export.py` remained argparse-based at the time, scheduled for marcos 4B/4C/4D. (Update 2026-08-20: `bcllm_run.py` and `bcllm_model.py` converted in marco 4B — see the dedicated entry below.)
...
### ✅ CLI Typer migration marco 4B — `bcllm_run.py` and `bcllm_model.py` converted from argparse

**Resolved:** 2026-08-20
**Description:** Completes marco 4B of the CLI Typer migration plan (`docs/architecture/adr/adr-002-cli-presentation.md`), in two slices per the user's explicit ordering (`bcllm_run.py` first — smaller, lower risk, validates the pattern before the larger, more sensitive `bcllm_model.py`). Both modules' `create_parser()` (argparse) removed entirely, replaced by real `typer.Typer` command definitions in `src/cli/commands/run.py` and `src/cli/commands/model.py` — every flag mirrored 1:1, invoked programmatically via `.main(standalone_mode=False)`, never exposed as a public Typer subcommand tree. External CLI syntax unchanged.
- **`bcllm_run.py` (first slice):** `handle_list_runs`/`handle_show_run`/`handle_remove_run` converted to typed params. `Event.RUN_CREATED` (existed, never wired) wired in `add_run_action`; `Event.RUN_REMOVED` (new) wired in `handle_remove_run`. `--randomization-seed`'s extra FORMAT check (`parse_randomization_seed_strict`) moved into `commands/run.py`'s command body, mirroring the mutex-group-check pattern (no single Click callback can express a cross-field check like this).
- **`bcllm_model.py` (second slice):** `handle_list_models`/`handle_remove_model` converted to typed params. `Event.MODEL_ADDED` (existed since marco 4A, wired only in `bcllm_experiment.py`'s `_add_models_at_creation`) is now ALSO wired in this module's own `add_model_action` — the specific gap flagged before starting this slice. Confirmed no double-emission: `bcllm.py`'s real composite flow (`--create-experiment --add-model`) calls `bcllm_model.run_add_model` directly (`bcllm.py::_execute_action_request`), never `bcllm_experiment.py::_add_models_at_creation` — module resolution routes any `--add-*` flag to its own module before `--create-experiment`'s context ever matters (`src/core/module_resolver.py`'s `ADD_ACTION_FLAGS` priority), so `_add_models_at_creation` is unreachable from the real `bcllm.py` entry point whenever `--add-model` is present; it was left as-is (not removed), a separate dead-code question outside this slice's scope. See `tests/unit/cli/test_model_logging_events.py` for the standalone + real-composite-flow proof.
  - `--reasoning` needed a different Typer construct than every other SUPPORTED string flag converted so far: it uniquely has BOTH an argparse `choices=[...]` constraint (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`/`system-default`) AND `SYSTEM_DEFAULT_SUPPORTED` membership. A plain `typer_str_or_system_default` callback would accept any string, silently dropping the choices restriction; native Typer/Click `choices=`/`Enum` validation alone would reject `'system-default'` before normalization could ever run. Resolved with a real `Enum` (`src/cli/commands/model.py::ReasoningEffort`) whose values include the literal `'system-default'`, converted to `FORCE_SYSTEM_DEFAULT` in the command body after Click's own enum-membership validation already ran — so an invalid value (including the deprecated `'null'`, never a valid choice for this flag) is rejected the same way argparse's `choices=` always rejected it: as an invalid choice, not via the generic deprecated-`'null'` message every other SUPPORTED flag shows.
  - `handle_remove_model`'s hard-delete behavior is unchanged and NOT reconsidered here — a prior, still-standing project decision (FK protection on `responses`/`errors` already guarantees it can never destroy actual results). Found and fixed, in the same pass, a documentation drift unrelated to this decision: both the module docstring and `commands/model.py`'s `--remove-model` help text said "(soft delete)", left over from before the 2026-08-17 decision that accepted the hard delete as-is (see the `--remove-experiment` entry below) — corrected to state the hard-delete is intentional.
  - Mechanical preflight (Haiku agent, before implementation) found: only the already-known `--output` dead flag as undocumented-but-declared; no orphaned flags, dead routing duplicates, or stray direct `create_parser` imports beyond the one expected test file. No new semantic decision was needed, so no drift required a stop-and-ask.
  - `tests/cli_suite/runner/coverage.py::discover_cli_flags()` silently dropped a module's flags from the coverage report the instant it converted to Typer (its `hasattr(module, "create_parser")` check just `continue`d past Typer modules with no fallback) — a real, compounding gap introduced starting with marco 4A's `bcllm_questions.py`/`bcllm_experiment.py`, not caught until now. Fixed to introspect the Typer command's Click params the same way `test_system_default_classification_consistency.py::declared_dests()` already does, for both `bcllm_run.py` and `bcllm_model.py` (and retroactively for `bcllm_questions.py`/`bcllm_experiment.py`).
**Verification:** New tests this slice: `test_commands_run.py` (29), `test_run_logging_events.py` (4), `test_run_standalone_composite_parity.py` (12), `test_commands_model.py` (80), `test_model_logging_events.py` (4), `test_model_standalone_composite_parity.py` (11); `test_bcllm_model_system_default.py` reduced from its former parser-dependent suite to the 5 ConfigResolver-domain-level tests that don't touch `create_parser()` (the CLI-parsing coverage it used to provide moved into `test_commands_model.py`). `pytest` final: **48 failed / 1419 passed / 18 skipped / 39 errors** — identical failed/skipped/error counts to every prior baseline this session, zero regressions. `cli_suite --profile full`: **58 cases, 57 PASS + 1 EXPECTED_FAILURE**, byte-identical, including all `AM-001`..`AM-016` and `RN-001`..`RN-011` cases green (the previously-flagged, unreproduced `RN-003` flake did not recur on this run — left as untouched evidence, per standing instruction not to alter tests to accommodate an unreproduced flake). Environment unchanged: **Python 3.14.2**, **Typer 0.27.1**, **Rich 14.3.4**.
**Impact:** Marco 4B complete. `bcllm_provider.py`, `bcllm_execute.py`, `bcllm_review.py`, `bcllm_export.py` remain argparse-based, scheduled for marcos 4C/4D.

### ✅ Reasoning effort/tokens exclusivity, `--max-reasoning` removal, and 3 related `system-default` inheritance bugs

**Resolved:** 2026-08-21 (user decision, definitive — full investigation and normative proposal presented and confirmed before implementation)
**Description:** Triggered by the `--reasoning-tokens 0`/`system-default` falsy-collapse bug found during the marco 4B Guardian review (see the Bugs section pointer above). Investigation established: `--max-reasoning` and `--reasoning-tokens` were true, undocumented synonyms — identical help text on both `bcllm_model.py` and `bcllm_experiment.py`, both feeding the exact same `MODEL_MAX_TOKENS_REASONING` key via a since-removed `getattr(cli_args, 'reasoning_tokens', None) or getattr(cli_args, 'max_reasoning', None)` in `ConfigResolver` (two call sites: `build_experiment_config_dict`, `build_model_config_dict`). `configuration-hierarchy.md`'s own normative flag list never named `max_reasoning` at all. Per OpenRouter's contract (`docs/Manuais_Diversos/openrouterdocs/reasoning_tokens.md`, "One of the following (not both)"), `reasoning.effort` and `reasoning.max_tokens` are mutually exclusive, and a deeper bug was found: `Planner._build_model_config` collapsed a persisted `MODEL_REASONING_EFFORT="none"` to Python `None` (`has_reasoning = reasoning_effort is not None and reasoning_effort != "none"`), which silently defeated `--reasoning none` whenever `MODEL_MAX_TOKENS_REASONING` was also present (inherited or explicit) — the downstream `Event.REASONING_CONFLICT` check never fired (it only compared `reasoning_effort is not None`, already collapsed), and `reasoning.max_tokens` was sent to the API despite the user explicitly asking to disable reasoning.

**Removed:** `--max-reasoning` — no alias, no deprecation shim, no compatibility path. Deleted from `src/cli/commands/model.py`/`experiment.py` (Typer option + dataclass field), `src/cli/bcllm_model.py`/`bcllm_experiment.py` (`SYSTEM_DEFAULT_SUPPORTED`, `AddModelRequest`), `ConfigResolver`'s two `or`-fallback call sites, `bcllm.py`'s composite-flow `relevant_flags["--add-model"]` list, `tests/cli_suite/cases/model.yaml::AM-008`, `docs/contracts/system-default-semantics.md`'s SUPPORTED table.

**New normative contract (user-specified, `docs/contracts/system-default-semantics.md` updated — "Reasoning Effort/Tokens Exclusivity" section):**
- Same-layer conflict (both `--reasoning` and `--reasoning-tokens` concrete on the same `--create-experiment`/`--add-model` command) is a usage error, exit 2 — checked in `src/cli/commands/{model,experiment}.py`'s command bodies, before any DB connection.
- A concrete value for either field (including `--reasoning none`, a real mode selection, not an absence) suppresses the OTHER field's inheritance entirely, even across layers — `ConfigResolver._resolve_reasoning_pair` (new shared method, used by both `build_experiment_config_dict` and `build_model_config_dict`).
- `system-default` is explicitly NOT a mode selection — it clears only its own field; a validly-inherited/independently-resolved sibling field is untouched.
- `--reasoning-tokens` rejects `0` and negative values as a usage error (exit 2), not silently `None` — Anthropic's documented floor is 1024 tokens regardless of what's requested, so persisting a literal `0` would misrepresent what execution actually does; `--reasoning none` already exists as the honest way to disable reasoning.
- `Planner._build_model_config` no longer collapses `"none"` — it passes `MODEL_REASONING_EFFORT` through exactly as persisted, and now computes `reasoning_mode` as `"effort"`/`"budget"`/`"off"` (a previously dead `ModelConfig.reasoning_mode` field, never consulted downstream, now given real meaning for the first time — no new consumer wired).
- `src/api/request_payload.py::build_chat_completion_payload`'s existing `elif`-based priority (effort wins if both non-`None`) was deliberately KEPT UNCHANGED — reframed in its own comment as a defensive fallback for pre-fix historical data (not migrated, per ADR-003 pre-production data scope), not the primary enforcement mechanism, which now lives upstream. `execution_engine.py`'s `Event.REASONING_CONFLICT` WARNING (unchanged) is kept as a non-fatal defensive check for historical rows.

**Guardian-found gap, fixed same day:** the initial version of this checkpoint left one real path where the same-layer CLI conflict check and `_resolve_reasoning_pair`'s mode-suppression could both be bypassed — a `.env` file setting BOTH `MODEL_REASONING_EFFORT` and `MODEL_MAX_TOKENS_REASONING`, with NEITHER `--reasoning` nor `--reasoning-tokens` passed on the CLI at `--create-experiment` time. Neither field is CLI-concrete in that case, so mode-suppression never fires, and the CLI-level same-layer check (which only inspects raw CLI values) never sees it either — both values would persist non-null onto the experiment, and propagate to every model_variant that doesn't override them. Confirmed live (not speculative) by the Guardian and independently corroborated by `test_planner_maps_all_config_keys`, which already proved `Planner._build_model_config` performs no exclusivity check of its own (by design — enforcement is meant to live at creation time, not at read time). **Fix:** `build_experiment_config_dict` now raises `ValueError` (surfaces as exit 1, same mechanism as its existing "experiment already exists" check) when both fields resolve non-null with neither CLI-concrete — the one place `.env` is actually consulted, so this closes the gap for every NEW experiment; `build_model_config_dict` is deliberately NOT given the same hard check, since it never reads `.env` and blocking `--add-model` against an experiment created before this fix would be more disruptive than the existing WARNING-level defense-in-depth already handles. `execution_engine.py`'s `Event.REASONING_CONFLICT` comment corrected — it is unreachable only for experiments created after this fix, not an unconditional "after the main fix" claim as the entry originally (inaccurately) stated. New tests: `tests/test_config_resolver.py::TestReasoningModeSuppression` (+3: both-set-in-.env rejected, mixed .env+CLI resolved correctly by suppression not the guard, single-field-in-.env not rejected). Two pre-existing tests (`test_returns_dict_with_17_keys`) had both fields incidentally set in their `.env` fixture (irrelevant to what the test actually verifies — dict shape/keys, not reasoning values) — adjusted to avoid tripping the new guard.

**Two additional divergences found by a targeted system-default audit** (requested by the user as part of this same checkpoint, covering every other SUPPORTED parameter against the 6-invariant checklist — see the updated `system-default-semantics.md` for the full "What 'Not Sent' Actually Means" / "API vs. Local Configuration" / "Configuration That Alters Request Structure" sections this audit produced), both fixed in the same pass:
1. **`--system-prompt`/`--user-prompt` system-default silently defeated at execution time.** `Planner._resolve_prompts_effective` had a SECOND, redundant `run_config.get("SYSTEM_PROMPT") or exp_config.get("SYSTEM_PROMPT")` fallback — `ConfigResolver.build_run_config_dict` already correctly bakes inheritance into the run's own config at creation time (Run configuration is frozen at creation, `configuration-hierarchy.md`), so this execution-time fallback was pure redundancy in the normal case, but actively wrong for `system-default`: a run explicitly created with `--system-prompt system-default` persists `SYSTEM_PROMPT: null` on purpose, and the redundant `or` re-inherited the experiment's prompt anyway, indistinguishable from "never resolved." Fixed: `_resolve_prompts_effective` now reads the run's own frozen config directly, no second inheritance step. `tests/unit/core/test_planner.py`'s two existing tests exercising this (via `RunFactory`, which — unlike the real `--add-run` pipeline — persists an explicit `None` rather than simulating real creation-time inheritance) were testing the buggy fallback; rewritten to (a) prove a run's own distinct prompt is used, (b) prove a run that inherited-at-creation-time (simulated by passing the experiment's prompt straight into the factory, as the real resolver would persist it) is read correctly with no further inheritance, and (c) a new test proving a run with an explicitly-cleared prompt does NOT get the experiment's re-applied.
2. **`--vision`/`--structured` inheritance from experiment to model_variant was completely broken** whenever not re-specified at `--add-model` time. `ConfigResolver._parse_bool_value` only recognized `str`/`FORCE_SYSTEM_DEFAULT` input, falling through to `return None` for a real Python `bool` — which is exactly what `exp_config.get("MODEL_VISION")` is after the `config_json` JSON round-trip when `_resolve_cli_or_experiment` re-applies the same parser to the inherited value. Every model_variant that didn't explicitly repeat `--vision`/`--structured` silently got `None` (behaves as `False` at both current consumption sites, so no crash, but a real, silent violation of the "system-default interrupts only its own field's inheritance, ordinary inheritance still works" invariant — the experiment-level default for these two flags was effectively dead). Fixed: `_parse_bool_value` now returns a real `bool` input unchanged.

**Verification:** New/updated tests: `tests/unit/cli/test_commands_model.py` (+12: `TestReasoningTokensPositiveIntOnly`, `TestReasoningEffortTokensSameLayerConflict`; `--max-reasoning` removed from `TestIntFlagsSystemDefault`), `tests/unit/cli/test_commands_experiment.py` (+7, same two classes), `tests/test_config_resolver.py` (+15, `TestReasoningModeSuppression` — 12 covering mode-suppression, +3 covering the Guardian-found `.env`-both-set gap; 6 dead `max_reasoning = None` MockArgs attributes removed; 2 pre-existing fixtures adjusted to not incidentally trip the new `.env` guard), `tests/test_request_config_application.py` (2 tests rewritten to call the real `Planner._build_model_config` instead of reimplementing its mapping logic inline — the reimplementation had been masking the `"none"`-collapse bug), `tests/unit/core/test_planner.py` (+1 new, 2 rewritten), `tests/cli_suite/cases/model.yaml` (AM-008 updated, +AM-017 same-layer conflict, +AM-018 zero-rejection, both real-subprocess). `pytest` final: **48 failed / 1448 passed / 18 skipped / 39 errors** — identical failed/skipped/error counts to every prior baseline, zero regressions. `cli_suite --profile full`: **60 cases, 59 PASS + 1 EXPECTED_FAILURE**, byte-identical apart from the 2 new AM-017/AM-018 cases. Reviewed by Essence Guardian twice — first pass found the `.env`-both-set gap (fixed same day, see above) and a documentation-lag warning (`docs/reference/cli-commands.md`/`configuration-reference.md` updated with the exclusivity rule); second pass not yet re-run as of this writing.
**Impact:** Reasoning configuration now has a single, unambiguous flag pair with a real exclusivity contract instead of two synonymous flags and a silent priority rule. `--reasoning none` is now honest end-to-end. Experiment-level `--vision`/`--structured` defaults now actually reach model variants that don't override them. Run-level `system-default` on prompts now actually breaks inheritance at execution time, not just at creation time.
**Not fixed by this change:** the broader system-default audit's remaining rows (all other SUPPORTED parameters: `--temperature`/`--top-p`/`--top-k`/`--repeat-penalty`/`--max-tokens`/`--model-seed`/`--provider`/`--randomization-seed`/`--provider-lock`/`--add-questions`/`--where`/`--exclude`) were verified OK, no divergence found — not re-litigated here.

### ✅ Python compatibility floor raised to >=3.14; two unrelated objectively-incorrect annotations fixed; test-only `click` import removed

**Resolved:** 2026-08-21 (user decision — a marco 4C read-only external audit of commit `263854e` surfaced these; addressed before marco 4C implementation begins, per the audit's own reconciliation request)
**Python floor:** `setup.py`'s `python_requires` raised from `>=3.10` (an unverified compatibility claim — see the dependency-hygiene entry above) to `>=3.14`, matching the only version this project has ever actually run/tested (`3.14.2`). `docs/architecture/overview.md` and `docs/guides/ai-development-workflow.md` updated (`Python >=3.14 (validated: 3.14.2)`). No version matrix was run — this narrows the claim to match reality, not a compatibility effort.
**`RetryHandler` scope bug (`src/core/async_orchestrator.py`):** `AsyncOrchestrator._execute_item_with_semaphore`'s `retry_handler: RetryHandler` parameter annotation referenced a name that was only ever imported LOCALLY inside a *different* method (`_execute_run_items`'s `from src.core.retry import RetryHandler`, used to construct the handler passed into `_execute_item_with_semaphore`) — silent under Python 3.14's default lazy/deferred annotation evaluation (PEP 649, same class of bug as the `bcllm_experiment.py::argparse.Namespace` gap found during marco 4A), would raise `NameError` on any `inspect.signature()`/`typing.get_type_hints()` call. No circular-import reason for the local import (confirmed: `src/core/retry.py` imports nothing from `async_orchestrator.py` or anything that does). **Fix:** hoisted `from src.core.retry import RetryHandler` to the module's top-level imports; removed the now-redundant local import. Verified via `typing.get_type_hints()` on the affected method — resolves cleanly to `src.core.retry.RetryHandler`.
**`Exception | callable` invalid annotation (`tests/unit/core/test_execution_engine.py`):** a mock's `set_side_effect(self, effect: Exception | callable)` used the builtin function `callable` where a type was needed — `Exception | Callable` is what was meant. **Fix:** added `from typing import Callable`, changed the annotation to `Exception | Callable`.
**Test-only `click` import (G12, `tests/unit/cli/test_remove_question_removed.py`):** one test directly did `import click; ctx = click.Context(_command); help_text = _command.get_help(ctx)` to render `--help` output — `click` is a transitive/vendored dependency of Typer (0.27+), never a declared direct one (confirmed no `click` entry anywhere in the compiled `requirements.txt`, per the dependency-hygiene entry above). Per instruction, `click` was NOT added as a declared dependency to make this "correct" — the test was rewritten to use Typer's own public testing interface instead. **Fix:** replaced with `typer.testing.CliRunner().invoke(_app, ["--help"])`, asserting on `result.output`. Confirmed zero remaining `import click` anywhere in `src/`/`tests/` (repo-wide grep).
**Verification:** `tests/unit/cli/test_remove_question_removed.py` — 19/19 pass. `AsyncOrchestrator`/`bcllm_execute.py` import chain confirmed clean (`inspect.signature()`/`typing.get_type_hints()` on every class method in both modules, zero errors); full test collection unchanged (1553 tests, no new/hidden tests surfaced — these were introspection-time bugs, not collection-time ones). `tests/test_async_writer.py` — 16/16 pass (see the "Test-suite debt groups G1–G9" entry in the Bugs section above for the G8 rewrite this same audit prompted). `pytest` final: **42 failed / 1455 passed / 18 skipped / 39 errors** (down from the 48-failed baseline by exactly the 6 G8 `AsyncWriter` tests fixed, +1 net new test from splitting a task_done claim in two — no other change to the failed/skipped/error counts). `cli_suite --profile full`: **60 cases, 59 PASS + 1 EXPECTED_FAILURE**, unchanged. Python >=3.14 confirmed in every active source: `setup.py`, `docs/architecture/overview.md`, `docs/guides/ai-development-workflow.md`, `AGENTS.md`, `README.md` (repo-wide grep for `3.10` confirms nothing else remains outside `Arquivos_Mortos/`/`.archive/`/dated historical entries in this same file).
**Impact:** No behavior change for either annotation fix — both were introspection-time-only bugs (never hit during normal execution, since nothing in the hot path calls `inspect.signature()`/`typing.get_type_hints()` on these). The Python floor change is a documentation/metadata correction, not a functional one. The `click` fix removes a latent, undeclared dependency risk from the test suite.
**Not fixed by this change:** G1–G9 (see below) are registered as suite debt, not addressed here; G8 is investigated separately without a production fix in this pass.
...
**Superseded 2026-08-21:** `--max-reasoning` was removed from the system entirely (true, undocumented synonym of `--reasoning-tokens`) — see the "Reasoning effort/tokens exclusivity" entry below. This entry is kept for history only; the flag it describes no longer exists, so this specific gap can no longer occur.
