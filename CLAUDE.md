...
- `core/` — business logic: the execution pipeline (`planner.py`, `execution_engine.py`, `execution_plan.py`, `result_writer.py`, `async_orchestrator.py`, `async_writer.py`, `run_finalizer.py`), config resolution (`config_resolver.py`, `special_config_values.py`), CLI mode handling (`mode.py`, `mode_resolver.py`, `mode_matrix.py`, `module_resolver.py`), and utilities (`retry.py`, `answer_parser.py`, `question_loader.py`, `json_serializer.py`).
...
- **Pre-production data scope:** see `docs/architecture/adr/adr-003-pre-production-data-scope.md` — compatibility with existing local `data/bcllm.db` content is not required while the system is pre-production, but this does not relax immutability, auditability, or test/diagnostic isolation. Read the ADR before deciding a data-shape bug fix needs migration/detection tooling.
